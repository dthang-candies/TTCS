...
with st.sidebar:
    st.markdown(
        f'<div style="border-bottom:1px solid #2d3748;padding-bottom:1rem;margin-bottom:0.75rem;">'
        f'<p style="font-size:1.5rem;font-weight:700;color:#f8fafc;margin:0;line-height:1.3;letter-spacing:-0.01em;">'
        f"AI Phân tích thay đổi văn bản pháp lý</p>"
        f'<p style="font-size:1.05rem;color:#cbd5e1;margin:0.5rem 0 0;line-height:1.5;font-weight:500;">'
        f"Thực tập cơ sở nhóm 3</p></div>",
    ...
    with st.container():
        ...
        if DEMO_MODE:
            ...
            st.markdown(
                '<div class="demo-mode-pill" style="background:#064e3b !important;border-color:#059669 !important;'
                'color:#6ee7b7 !important;">PRODUCTION</div>',
                unsafe_allow_html=True,
            )
...
# ═══════════════════════════════════════════════════════════════════════════
# TOPBAR — Trạng thái kết nối | Kiểm tra | Dark/Light
# ═══════════════════════════════════════════════════════════════════════════

if DEMO_MODE:
    _status_label = "DEMO MODE"
    _status_color = "#818cf8"
else:
    _status_label = "Đã kết nối"
    _status_color = "#22c55e"

with st.container(key="hdr_toolbar"):
    _col_status, _col_check, _col_theme = st.columns([6, 2, 2], gap="small")

    with _col_status:
        st.markdown(
            f'<p style="margin:0;padding:6px 0;font-size:0.9rem;color:{_sub_c};">'
            f'<span style="display:inline-block;width:9px;height:9px;border-radius:50%;'
            f'background:{_status_color};margin-right:8px;vertical-align:middle;"></span>'
            f'Trạng thái: '
            f'<span style="font-weight:600;color:{_status_color};">{_status_label}</span>'
            f'</p>',
            unsafe_allow_html=True,
        )

    with _col_check:
        if st.button("Kiểm tra", key="hdr_check", type="secondary", use_container_width=True):
            if DEMO_MODE:
    ...
    with _col_theme:
        ...
        if st.button(_btn_theme, key="hdr_theme", type="secondary", use_container_width=True):
            st.session_state["ui_theme"] = "light" if _dark else "dark"
            st.rerun()

# Đường phân cách topbar
st.markdown(
    f'<hr style="border:none;border-top:1px solid {"#334155" if _dark else "#e2e8f0"};margin:0 0 1.25rem 0;">',
    unsafe_allow_html=True,
)
