...
class Comparator:
    ...
    def compare_all(self, pairs: List[MatchedPair]) -> List[ChangeItem]:
        ...
        logger.info(f"Comparator: {len(results)} changes")
        return results
    ...
    def _compare_one(self, pair: MatchedPair) -> List[ChangeItem]:
        ...
        # merge-aware case từ Matcher
        if pair.match_strategy and "merge" in pair.match_strategy:
            items.append(self._handle_merge(pair))
            return items
        ...
        if self._normalized_equal(ca.text, cb.text):
            return [ChangeItem(
                change_type=ChangeType.UNCHANGED,
                mo_ta="Nội dung tương đương sau chuẩn hóa",
                vi_tri=self._resolve_vi_tri(ca, cb),
                citation_a=None,
                citation_b=None,
                muc_do="thap",
        ...
        try:
            raw_items = self.llm.compare_chunks(ca.text, cb.text, context=context)
        except Exception as e:
            logger.warning(f"LLM error: {e}")
            return [self._fallback(ca, cb)]

        for d in raw_items:
            item = self._build_item(d, ca, cb)
            if item:
                items.append(item)

        if not items:
            items.append(self._fallback(ca, cb))
    ...
    def _handle_merge(self, pair: MatchedPair) -> ChangeItem:
        return ChangeItem(
            change_type=ChangeType.MODIFIED,
            mo_ta="Nội dung được gộp từ nhiều đoạn",
            vi_tri=self._resolve_vi_tri(pair.chunk_a, pair.chunk_b),
            citation_a=None,
            citation_b=None,
            muc_do="trung binh",
    ...
    def _build_item(self, d: dict, ca: Chunk, cb: Chunk) -> ChangeItem | None:

        mo_ta = d.get("mo_ta", "").strip()
        if not mo_ta:
            return None

        change_type = self._map_type(d.get("change_type"))

        return ChangeItem(
            change_type=change_type,
            mo_ta=mo_ta,
            vi_tri=self._resolve_vi_tri(ca, cb),
            citation_a=self._extract_citation(d.get("trich_dan_a"), ca, DocSource.A),
            citation_b=self._extract_citation(d.get("trich_dan_b"), cb, DocSource.B),
            muc_do=self._infer_severity(ca.text, cb.text),
        )
    ...
    def _map_type(self, t: str) -> ChangeType:
        t = (t or "").upper().strip()
        ...
        return {
            ...
            "DOI VI TRI": ChangeType.REORDERED,
            "KHONG DOI NOI DUNG": ChangeType.UNCHANGED,
        }.get(t, ChangeType.MODIFIED)

    # ─────────────────────────────────────────────
    # SEVERITY (FIXED LOGIC)
    # ─────────────────────────────────────────────

    def _infer_severity(self, a: str, b: str):

        a_n = _unicode_normalize(a)
        b_n = _unicode_normalize(b)

        ratio = SequenceMatcher(None, a_n[:1500], b_n[:1500]).ratio()

        if ratio >= self._low_ratio:
            return "thap", "Thay đổi nhẹ"

        if ratio >= self._med_ratio:
            return "trung binh", "Thay đổi vừa"

        return "cao", "Thay đổi lớn"

    # ─────────────────────────────────────────────
    # CITATION
    # ─────────────────────────────────────────────

    def _extract_citation(self, text: str, chunk: Chunk, source: DocSource):
        if not text or len(text.strip()) < self.min_citation_len:
            return None
    ...
    def _resolve_vi_tri(self, ca: Chunk, cb: Chunk) -> str:
        meta = ca.metadata if ca else cb.metadata

        parts = []

        if meta.dieu:
            parts.append(meta.dieu)
        if meta.khoan:
            parts.append(f"Khoản {meta.khoan}")
        if meta.diem:
            parts.append(f"Điểm {meta.diem}")

        return " > ".join(parts) if parts else (meta.heading_path or "")
    ...
    def _fallback(self, ca: Chunk, cb: Chunk) -> ChangeItem:
        return ChangeItem(
            change_type=ChangeType.MODIFIED,
            mo_ta="Nội dung thay đổi",
            vi_tri=self._resolve_vi_tri(ca, cb),
            citation_a=None,
            citation_b=None,
            muc_do="trung binh",
