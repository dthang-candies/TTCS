...
from schemas import Chunk, MatchedPair
...
class Matcher:
    ...
    def __init__(self):
        ...
        self.threshold = cfg.SIM_THRESHOLD
        self.merge_threshold = getattr(cfg, "MERGE_SIM_THRESHOLD", max(0.68, self.threshold - 0.08))
        self.top_k = getattr(cfg, "TOP_K_MATCH", 5)
        self.max_merge_window = getattr(cfg, "MAX_MERGE_WINDOW", 3)
    ...
    def match(
        ...
        # ─────────────────────────────────────────────
        vecs_a = np.array(self.embedder.encode_dense([c.text for c in chunks_a]))
        vecs_b = np.array(self.embedder.encode_dense([c.text for c in chunks_b]))

        sim_matrix = self._cosine_matrix(vecs_a, vecs_b)

        # ─────────────────────────────────────────────
        # STEP 2: TOP-K PRUNING
        # ─────────────────────────────────────────────
        allowed_edges = self._top_k_edges(sim_matrix, self.top_k)

        # apply mask (invalid edges → very low score)
        masked = np.full_like(sim_matrix, -1.0)
        for i, js in allowed_edges.items():
            for j in js:
                masked[i, j] = sim_matrix[i, j]

        # ─────────────────────────────────────────────
        # STEP 3: HUNGARIAN ASSIGNMENT
        # ─────────────────────────────────────────────
        cost_matrix = 1 - np.clip(masked, 0, 1)
        ...
        for i, j in zip(row_ind, col_ind):
            if i >= len(chunks_a) or j >= len(chunks_b):
                continue
        ...
        # ─────────────────────────────────────────────
        # STEP 4: MERGE RECOVERY (1→N / N→1)
        # ─────────────────────────────────────────────
        merge_pairs, ma2, mb2 = self._merge_recovery(
            chunks_a, chunks_b, vecs_a, vecs_b, matched_a, matched_b
        )
        ...
        for j, cb in enumerate(chunks_b):
            if j not in matched_b:
                pairs.append(MatchedPair(
                    ...
                    sim_score=0.0,
                    is_matched=False,
                    match_strategy="added"
                ))

        logger.info(
            f"Matcher: total={len(pairs)}, "
            f"matched={len(matched_a)}, "
            f"unmatched={len(chunks_a) - len(matched_a) + len(chunks_b) - len(matched_b)}"
        )

        return pairs

    # ─────────────────────────────────────────────
    # EMBEDDING SIM MATRIX
    # ─────────────────────────────────────────────
    ...
    def _top_k_edges(self, sim_matrix: np.ndarray, k: int) -> Dict[int, List[int]]:
        edges: Dict[int, List[int]] = {}
        for i in range(sim_matrix.shape[0]):
            top_j = np.argsort(sim_matrix[i])[::-1][:k]
            edges[i] = top_j.tolist()
        return edges
    ...
    def _merge_recovery(
        ...
        for i, ca in enumerate(chunks_a):
            ...
            for start in range(len(chunks_b)):
                for w in range(2, self.max_merge_window + 1):
                    idxs = list(range(start, min(start + w, len(chunks_b))))
                    if not idxs:
                        continue

                    merged_vec = np.mean(vecs_b[idxs], axis=0)
                    merged_vec = merged_vec / (np.linalg.norm(merged_vec) + 1e-8)

                    score = self.embedder.similarity(vecs_a[i], merged_vec)

                    if score > best_score:
                        best_score = score
                        best_group = idxs

            if best_group and best_score >= self.merge_threshold:
                pairs.append(MatchedPair(
                    chunk_a=ca,
                    chunk_b=self._merge_chunks([chunks_b[j] for j in best_group]),
                    sim_score=float(best_score),
                    is_matched=True,
                    match_strategy="merge_b"
                ))

                new_ma.add(i)
                new_mb.update(best_group)

        return pairs, new_ma, new_mb
    ...
    def _merge_chunks(chunks: List[Chunk]) -> Chunk:
        ...
        return Chunk(
            ...
            metadata=first.metadata.model_copy(update={
                "heading_path": " + ".join(c.metadata.heading_path for c in chunks)
            })
        )